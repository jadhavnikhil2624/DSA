class Solution {
  ...
  private boolean isBinaryTreeValid(
    int root,
    int[] leftChild,
    int[] rightChild
  ) {
    ...
    while (!nodeQueue.isEmpty()) {
      ...
      if (leftChild[current] != -1) {
        if (
          visited[leftChild[current]]
        ) return false; // Check for cycle
  ...
  public boolean validateBinaryTreeNodes(
    ...
    for (int child : leftChild) {
      ...
      if (child != -1) childCount[child] = true; // Mark left child as having a parent
    }
    ...
    for (int child : rightChild) {
      ...
      if (child != -1) {
        if (
          childCount[child]
        ) return false; // Check if the right child already has a parent
    ...
    for (int i = 0; i < n; ++i) {
      if (!childCount[i]) {
        if (root == -1) root = i; // Set root node if not assigned
        else return false; // Multiple roots found, not a valid binary tree
      }
    }
