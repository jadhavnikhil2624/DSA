...
class Solution {
  ...
  public int[] sortItems(
    ...
    for (int i = 0; i < n; i++) if (indegreeGroups[i] == 0) groupQueue.offer(i);
    ...
    while (!groupQueue.isEmpty()) {
      ...
      for (int item : groupItems) if (indegreeItems[item] == 0) itemQueue.offer(
        item
      );
      while (!itemQueue.isEmpty()) {
        ...
        for (int neighbor : v) {
          ...
          if (groupId != group[neighbor]) {
            if (--indegreeGroups[group[neighbor]] == 0) groupQueue.offer(
              group[neighbor]
            );
          } else {
            if (indegreeItems[neighbor] == 0) itemQueue.offer(neighbor);
          }
        }
      }
    }
  ...
  private void buildGraph(int n, int[] group, List<List<Integer>> beforeItems) {
    ...
    for (int i = 0; i < n; i++) {
      for (int j : beforeItems.get(i)) {
        if (group[i] != group[j]) indegreeGroups[group[i]]++;
        if (graph[j] == null) graph[j] = new ArrayList<Integer>();
        graph[j].add(i);
        indegreeItems[i]++;
      }
    }
  ...
  private void buildGroups(int n, int[] group) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (groups[group[i]] == null) groups[group[i]] = new ArrayList<Integer>();
